...
MAX_PAGES = 8000        # safety cap on page COUNT, not records - page size is now adaptive (see
                        # PAGE_SIZE_STEPS below), so a rough patch needing many small pages must
                        # not exhaust this before genuinely finishing all ~58k records
...
PAGE_SIZE_STEPS = [100, 20, 5, 1]  # tried in order on failure; recovery probes step back up through these
RECOVERY_STREAK = 100  # after this many consecutive successful pages at a reduced size, try stepping back up


def _set_url_size(url, new_size):
    """Swap the size= query param in a URL to a different value. Used to
    retry the exact same cursor position at a different page size."""
    return re.sub(r"size=\d+", f"size={new_size}", url)
...
def fetch_all_publications():
    ...
    Page size is adaptive, not fixed. NVA has been observed to return a
    persistent 500 for some pages at size=100 while the exact same position
    succeeds at size=1 - a batch-size/serialization issue on their end, not
    one corrupt record (confirmed: many consecutive positions after the
    first failure all worked fine at size=1). Dropping to size=1 for the
    rest of a ~58,000-record run is far too slow (tens of thousands of
    individual requests), so this steps down through PAGE_SIZE_STEPS on
    failure, and periodically probes back up to a larger size after a
    streak of successes, so most of the run stays fast and only the
    problematic stretches slow down.

    If even the smallest step in PAGE_SIZE_STEPS fails at some position,
    that is treated as a genuinely unretrievable record (not a batch-size
    problem) and the run aborts with the exact position, rather than
    guessing how to skip past unknown data."""
    records = []
    step_idx = 0
    consecutive_successes = 0
    base_params = f"institution={USN_INSTITUTION_ID}&sort=identifier"
    url = f"{API_BASE}/search/resources?{base_params}&size={PAGE_SIZE_STEPS[0]}"
    page = 0
    seen_page_sizes = set()
    max_step_idx_used = 0
    ...
    while url and page < MAX_PAGES:
        ...
        # Periodically probe back up to a larger size after a streak of
        # successes at a reduced size, so we don't stay slow forever.
        if step_idx > 0 and consecutive_successes >= RECOVERY_STREAK:
            probe_idx = step_idx - 1
            probe_url = _set_url_size(url, PAGE_SIZE_STEPS[probe_idx])
            try:
                data = fetch_json(probe_url)
                print(f"  Page {page}: recovered up to size={PAGE_SIZE_STEPS[probe_idx]} after {consecutive_successes} stable pages at size={PAGE_SIZE_STEPS[step_idx]}.", file=sys.stderr)
                step_idx = probe_idx
                consecutive_successes = 0
                url = probe_url
            except Exception:
                data = None  # probe failed, fall through and just continue at current step_idx below

        if data is None:
            current_size = PAGE_SIZE_STEPS[step_idx]
            attempt_url = _set_url_size(url, current_size)
            try:
                data = fetch_json(attempt_url)
                consecutive_successes += 1
                url = attempt_url
            except Exception as e:
                found = False
                for next_idx in range(step_idx + 1, len(PAGE_SIZE_STEPS)):
                    smaller_size = PAGE_SIZE_STEPS[next_idx]
                    smaller_url = _set_url_size(url, smaller_size)
                    print(f"  Page {page} failed at size={current_size}. Trying size={smaller_size} at the same position...", file=sys.stderr)
                    try:
                        data = fetch_json(smaller_url)
                        step_idx = next_idx
                        max_step_idx_used = max(max_step_idx_used, step_idx)
                        consecutive_successes = 0
                        url = smaller_url
                        found = True
                        break
                ...
                if not found:
                    raise RuntimeError(
                        ...
                        f"and it still fails even at the smallest page size ({PAGE_SIZE_STEPS[-1]}) - "
                        f"this looks like one specific record NVA itself can't serve, not a "
                        f"batch-size problem.\nLast URL tried: {url}\n"
        ...
        if page == 1:
            ...
            print(f"Reported totalHits: {total}. Page size {PAGE_SIZE_STEPS[step_idx]}, got {len(hits)} on first page.")
        if page <= 5 or page % 50 == 0:
            print(f"  ...page {page}, {len(records)} records so far, current page size {PAGE_SIZE_STEPS[step_idx]}.")
    ...
    if max_step_idx_used > 0:
        print(f"NOTE: this run had to step down to a smaller page size at least once (max reduction: size={PAGE_SIZE_STEPS[max_step_idx_used]}) due to a batch-size issue on NVA's end, with automatic recovery back up when stable.")
    if page >= MAX_PAGES:
        raise RuntimeError(
